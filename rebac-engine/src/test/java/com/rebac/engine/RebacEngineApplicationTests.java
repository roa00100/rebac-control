...
import com.rebac.engine.adapter.in.web.RelationshipRestController;
import com.rebac.engine.adapter.in.web.RestExceptionHandler;
...
class RebacEngineApplicationTests {
    ...
    void delegatesIngestAndCheckToService() throws Exception {
        ...
        mockMvc.perform(post("/api/v1/tuples")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content("{\"mode\":\"replace\",\"tuples\":[]}"))
                .andExpect(status().isNoContent());
        verify(rebacService).ingestTuples(eq("replace"), eq(List.of()));
