package com.rebac.engine.adapter.in.web;
...
import com.rebac.engine.adapter.in.web.dto.CheckRequestDto;
import com.rebac.engine.adapter.in.web.dto.CheckResponseDto;
import com.rebac.engine.adapter.in.web.dto.TupleBatchRequest;
import com.rebac.engine.adapter.in.web.dto.TupleDto;
...
public class RelationshipRestController {
    ...
    public EngineMeta meta() {
        return new EngineMeta(
                "rebac-engine",
                "REST → rebac-core(RebacService) → RebacEngine SPI. OpenFGA는 rebac-openfga-adapter 자동구성.");
    }
    ...
    public void ingestTuples(@Valid @RequestBody TupleBatchRequest body) {
        ...
        var dtoTuples = body.tuples() == null ? Collections.<TupleDto>emptyList() : body.tuples();
        List<RelationshipTuple> tuples = dtoTuples.stream().map(RelationshipRestController::toDomain).toList();
